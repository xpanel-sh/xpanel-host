...
- El gestor de archivos del dominio principal presenta su familia como carpetas hermanas con nombres completos —principal y subdominios— y excluye dominios no asociados. Las raíces físicas también son ahora hermanas por FQDN; la actualización migra el formato anidado anterior y una carpeta de proyecto llamada `subdomains` vuelve a ser contenido normal. iKode prepara automáticamente el ACL de cada raíz al entrar y repara de forma puntual el directorio de una operación si perdió acceso.
- iKode incorpora selección múltiple tipo explorador mediante casillas, `Ctrl/Cmd`, `Shift`, `Ctrl/Cmd+A` y marco de arrastre; un clic selecciona, doble clic abre y la eliminación conjunta confirma una sola vez, muestra progreso y conserva seleccionados únicamente los elementos que fallen.
- La sincronización de raíces se autorrepara si la base de datos ya apunta al FQDN plano pero los archivos continúan en el árbol anidado heredado. También reemplaza de forma segura una raíz plana vacía creada durante un intento anterior, pero detiene la migración si el destino contiene datos.
- Las instalaciones que llegaron a mostrar dominios desde el sandbox local `storage/app/sites` o conservan raíces bajo una identidad de cuenta anterior recuperan ese contenido hacia su FQDN actual durante la actualización. iKode ya no disfraza una raíz física ausente: la marca como pendiente de sincronización. La terminal del dominio principal abre una carpeta `/family` con el dominio y sus subdominios como hermanos, montados explícitamente dentro de su cárcel y sin exponer otros dominios de la cuenta.
- La sincronización prepara primero los pools PHP-FPM y servicios Node.js de todos los sitios y recién después valida o reinicia runtimes. Así, un `chdir` o `WorkingDirectory` heredado de la antigua carpeta `subdomains` no puede detener la actualización antes de que los demás dominios reciban su nueva raíz plana.
- La reconstrucción de SSH/SFTP/terminal ya no intenta cambiar con `usermod -d` el home de una identidad que mantiene procesos web activos. La cárcel monta de forma compatible cualquier home heredado y el perfil entra inmediatamente al workspace canónico, sin detener PHP-FPM, Node.js ni sesiones abiertas. PHP-FPM, Node.js y la terminal reciben también un `HOME` explícito con la raíz FQDN vigente, evitando escrituras en la antigua carpeta `subdomains`.
...
- La sincronización migra las raíces de dominios principales desde `/var/www` o `/srv/www` al hogar real de la cuenta y procesa primero todos los movimientos antes de regenerar servicios.
- Dominios y subdominios viven como raíces independientes `public_html/<fqdn>`; la sincronización migra tanto raíces externas heredadas como el formato anidado anterior.
