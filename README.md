...
- En la terminal web, ejecutar `npm start`, `npm run start`, `npm run serve`, `npm run production` o una entrada convencional como `node server.js` no crea un proceso temporal: XPanel identifica el sitio por el directorio actual, registra el comando permitido, prepara dependencias y reinicia su servicio systemd. El resto de npm y Node.js sigue funcionando directamente para instalar, compilar, probar o ejecutar herramientas. La capacidad dura ocho horas y sólo puede administrar el sitio —o familia/cuenta— autorizado para esa terminal.
...
La sincronización diaria de SSL no accede directamente a `/etc/letsencrypt` desde Laravel. Solicita al helper privilegiado únicamente estado, emisor y vencimiento —nunca la clave privada—, repara automáticamente estados `error` cuando el certificado sigue vigente y no invalida HTTPS por una interrupción temporal de sudo o del broker. Certbot conserva su timer de renovación y su deploy hook recarga los servicios y vuelve a sincronizar los metadatos.
...
La pestaña **Agentes** de Ikode permite registrar conexiones personales para Codex/OpenAI y Claude/Anthropic mediante clave API. La clave queda cifrada con `APP_KEY`, nunca regresa al navegador y sólo su propietario puede usarla o eliminarla. Todo permanece dentro del panel derecho de Ikode: pestañas de agentes, conversación, historial, nuevo chat y ajustes. Una conexión puede mantener varios chats, con historiales independientes para la cuenta completa y para cada sitio: Ikode general genera contexto únicamente desde el hogar de alojamiento, mientras Ikode de dominio queda confinado al document root de ese sitio.
